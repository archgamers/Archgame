...
def main():  # - общий план того что делает программа
    ...
    while winner is None:
        ...
        for b in boards:
            ...
            # Так как BankruptEvent устанавливает лимит на следующий спринт,
        ...
        # Наводим порядок
        # Проверка после эвента: тянет ли своих пользователей после действий
        # ВСЕХ теперь его конструкция
        for num in range(len(boards)):
            boards[num].users = min(
                boards[num].users, boards[num].cap(
                    boards[num].quantity_component(
                        constants.API), boards[num].quantity_component(
                        constants.DB), boards[num].quantity_component(
                        constants.LB)))
